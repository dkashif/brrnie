...
class FridgeItemSerializer(serializers.ModelSerializer):
    class Meta:
        model = FridgeItem
        fields = '__all__'
        read_only_fields = ('date_added', 'is_expired')
    ...
    def validate_quantity(self, value):
        if value < 0:
            raise serializers.ValidationError("The quantity cannot be a negative number.")
        if value == 0:
    ...
    def validate_expiration_date(self, value):
        if value <= timezone.now().date():  # Check if the expiration date is in the future
            raise serializers.ValidationError("Expiration date must be a future date. Please check your input.")
        return value
